class ChatGetter {
    ...
    static badgesDict = {
        ...
        "patner" : "https://static-cdn.jtvnw.net/badges/v1/d12a2e27-16f6-41d0-ab77-b780518f00a3/1"
    };
    ...
    static createChatElement(node) {
        ...
        chatLine.classList.add("chat-line");
            ...
            contentContainer.classList.add("content-container");
                if (node.message.userBadges != null) {
                    var badges = this.getBagdes(node.message.userBadges);
                }
                var username = document.createElement("span");
                username.classList.add("username");
                if (node.commenter != null) {
            ...
            if (badges != null) {
                contentContainer.appendChild(badges);
            }
    ...
    static getBagdes(badgesNode) {
        var badgesContainer = document.createElement("div");
        badgesContainer.classList.add("badges-container");

        badgesNode.forEach(badgeNode => {
            if (this.badgesDict[badgeNode.setID] != null) {
                var badgeImg = document.createElement("img");
                badgeImg.classList.add("chat-badge");
                badgeImg.src = this.badgesDict[badgeNode.setID];
    ...
    async getNextMessages(videoTime) {
        ...
        if (videoTime > 10) {
            if (this.messages.length == 0 || this.messages[this.messages.length - 1].timestamp < videoTime  || this.messages[0].timestamp > videoTime + 1) {
                this.messages = [];
    ...
    async updateMessages(videoTime) {
        ...
        if (this.messages.length == 0 && !this.isUpdating) {
            // this.isUpdating = true;
            await this.getMessagesFromSecOffset(Math.floor(videoTime));
            // this.isUpdating = false;
        }

        // if (this.messages[0].timestamp > videoTime + 1) {
        //     // this.isUpdating = true;
        //     await this.getMessagesFromSecOffset(Math.floor(videoTime));
        //     // this.isUpdating = false;
        // }
